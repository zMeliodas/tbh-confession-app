      ...
      <nav className="bg-backgroundColor flex justify-between px-4 md:justify-around items-center w-full py-1 fixed h-22">
        ...
        <div className="hidden md:flex gap-3 items-center">
          ...
          <Link to={isAuthenticated ? "/profile" : "/login"}>
            <NavButton rounded="rounded-xl" width="w-26" height="h-20" title="Profile">
          ...
          <Link to={isAuthenticated ? "/whisper" : "/login"}>
            <NavButton rounded="rounded-xl" width="w-26" height="h-20" title="Whisper">
              <TiMessages className="w-8 h-8" />
            </NavButton>
          </Link>

          <Link to={isAuthenticated ? "/settings" : "/login"} >
            <NavButton rounded="rounded-xl" width="w-26" height="h-20" title="Settings">
              {isAuthenticated ? (
